...
<title>Create your account · WhatsDesk</title>
...
<style>
  ...
  *{ -webkit-font-smoothing:antialiased; }
  ...
  .panel .brand, .panel h1{ animation:fadeIn .7s ease both; }
  @keyframes fadeIn{ from{ opacity:0; } to{ opacity:1; } }
  @media (prefers-reduced-motion: reduce){
    .login-card, .feat, .panel .brand, .panel h1, .panel::after{ animation:none; }
  }
  @media (min-width:992px){ .panel{ display:flex; } }
  .panel .brand{ display:flex; align-items:center; gap:.7rem; }
  .brand-mark{ width:40px; height:40px; border-radius:12px; overflow:hidden; display:grid; place-items:center; }
  ...
  .login-card{ width:100%; max-width:420px; animation:cardIn .6s cubic-bezier(.22,.9,.3,1) both; }
...
<div class="split">
  ...
  <main class="formside">
    <div class="login-card">
      ...
      <p class="text-secondary mb-4" style="font-size:.9rem;">One account, your business. Set your own login — it takes 30 seconds.</p>
      ...
      <?= form_open('admin/auth/register') ?>
        <div class="mb-3">
          ...
          <input type="text" name="username" class="form-control" autocomplete="username" required autofocus
                 placeholder="e.g. yourbusiness" minlength="3" maxlength="30">
        </div>
        <div class="mb-3">
        ...
        <div class="mb-4">
          ...
          <div class="input-group">
            <input type="password" name="confirm_password" class="form-control" autocomplete="new-password" required minlength="8">
      ...
      <div class="text-center mt-4" style="font-size:.85rem;">
        Already set up? <a href="<?= site_url('admin/auth/login') ?>">Sign in instead</a>
      </div>
      <div class="text-secondary text-center mt-2" style="font-size:.78rem;">
        After this you'll be guided through connecting WhatsApp and the AI — step by step.
      </div>
    </div>
  </main>
</div>
